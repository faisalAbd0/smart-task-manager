...
class TaskControllerTest {
    ...
    void whenFindAllTasks_thenReturnPageOfTaskResponses() throws Exception {
        ...
        Mockito.when(taskService.findAll(Mockito.any(), Mockito.any())).thenReturn(page);
    ...
    void givenCreateTaskRequest_whenPostTask_thenReturnCreatedTask() throws Exception {
        ...
        Mockito.when(taskService.addTask(Mockito.any())).thenReturn(response);
    ...
    void givenUpdatedTaskRequest_whenPutTask_thenReturnUpdatedTask() throws Exception {
        ...
        TaskResourceResponse response = TaskResourceResponse.builder()
                ...
                .title(request.getTitle())
                .build();
        Mockito.when(taskService.updateTask(Mockito.any())).thenReturn(response);

        mvc.perform(MockMvcRequestBuilders.put("/api/v1/tasks")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.taskId").value(request.getTaskId()))
    ...
    void whenDeleteTask_thenReturnOk() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.delete("/api/v1/tasks")
                        .param("taskId", taskId))
                .andExpect(status().isOk());
